...
mod YieldToken {
    ...
    struct Storage {
        ...
        _updates: LegacyMap<ContractAddress, u64>,
    ...
    fn _balance_of(account: ContractAddress) -> u256 {
        // TODO very rudimentary yield increase calculation.
        // It should be upgraded to something more sophisticated.
        // Only used to simulate a yield-bearing protocol.
        let last_update = _updates::read(account);
        let deltaT: u64 = get_block_timestamp() - last_update;
        let elapsed_periods: felt252 = (deltaT % (60 * 60 * 24)).into(); // 1% per day
        if elapsed_periods == 0 {
            return _balances::read(account);
        }
        let elapsed_periods: u256 = elapsed_periods.into(); // 1% per day
        let scaled_balance: u256 = _balances::read(account);
        (scaled_balance * 101.into() * elapsed_periods / 100.into())
    }
    ...
    fn _burn(account: ContractAddress, amount: u256) {
        ...
        _total_supply::write(_total_supply::read() - amount);
        _balances::write(account, _balance_of(account) - amount);
        _updates::write(account, get_block_timestamp());
    ...
    fn _transfer(sender: ContractAddress, recipient: ContractAddress, amount: u256) {
        ...
        _balances::write(sender, _balance_of(sender) - amount);
        _balances::write(recipient, _balance_of(recipient) + amount);
        _updates::write(sender, get_block_timestamp());
        _updates::write(recipient, get_block_timestamp());
